...
\paragraph{Role of the time-to-failure head.}
The $ \hat{\tau}_t $ head is not used only as an auxiliary metric: it regularizes the latent representation toward temporal hazard structure. In practice, lower predicted $ \hat{\tau}_t $ correlates with imminent contact-risk states and encourages earlier interventions. This enables preemptive corrections before terminal failure, reducing reliance on last-step, high-magnitude action jerks.
