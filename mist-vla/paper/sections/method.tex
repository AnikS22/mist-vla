...
\subsection{Final Metric Aggregation Protocol}
All final paper metrics are computed from completed JSON result files in \texttt{paper/data} by pooling raw counts, not averaging best runs.
For each family and controller mode, we aggregate:
\begin{equation}
\mathrm{Success\%} = 100 \cdot \frac{\sum_i n^{(i)}_{\mathrm{success}}}{\sum_i n^{(i)}_{\mathrm{episodes}}}.
\end{equation}
This prevents small-run bias and avoids cherry-picking favorable seeds. Latency is reported as the mean of logged per-task \texttt{mean\_apply\_ms} values across completed runs.
...
To avoid overclaiming, embodied Yahboom conclusions are reported separately from simulation and only after repeatable physical-trial logs are complete.
